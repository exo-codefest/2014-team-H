...
function AddressPicker(option){
    ...
    function map_init(map_canvas){
        ...
        set_position(new google.maps.LatLng(39.904214, 116.407413))
        ...
        function geodecode(latLng){
            ...
            geocoder.geocode({'latLng': latLng}, function(results, status){
                ...
                var zipCode;
                var country;
                //alert(results[0].address_components[0].types);
                if(results[0].types.indexOf('street_address') > -1)
                {
                    for (var i=0;i<results[0].address_components.length;i++)
                    {
                        if(results[i].address_components[0].types.indexOf('street_number') > -1) streetNumber=results[i].address_components[0].short_name;
                        if(results[i].address_components[0].types.indexOf('route') > -1) route=results[i].address_components[0].short_name;
                        if(results[i].address_components[0].types.indexOf('locality') > -1) city=results[i].address_components[0].short_name;
                        if(results[i].address_components[0].types.indexOf('administrative_area_level_1') > -1) state=results[i].address_components[0].short_name;
                        if(results[i].address_components[0].types.indexOf('postal_code') > -1) zipCode=results[i].address_components[0].short_name;
                        if(results[i].address_components[0].types.indexOf('country') > -1) country=results[i].address_components[0].short_name;
                    }
                }
                var str = '<div>';
                ...
                node.find('button.ok').click(function(){
                    ...
                    var ret = option.callback(latLng, addr, streetNumber, route, city ,state ,zipCode ,country);
                    if(ret !== false){
                        self.close();
                    }
